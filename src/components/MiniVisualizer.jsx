    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 mt-14 mb-20">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="hidden md:flex items-center gap-2">
          <Link
            to={`/room-visualization?room=${room.key}&colors=${encodeURIComponent(combo.colors.join(","))}`}
        ...
        {rooms.map((r, i) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-sm border ${
              i === roomIdx ? "bg-[#F0C85A] border-[#F0C85A] text-[#493657]" : "bg-white border-gray-200 text-[#493657]"
            }`}
            title={r.title}
          >
            {r.title}
      ...
      <div className="grid md:grid-cols-2 gap-8 items-center">
        ...
        <div className="relative overflow-hidden shadow-xl border border-black/5 bg-white ">
          <img
            src={combo.image || room.imageFallback}
            alt={room.title}
            width="900"
            height="600"
            loading="eager"
            className="w-full h-auto select-none"
            onError={(e) => { e.currentTarget.src = room.imageFallback; }}
          />
