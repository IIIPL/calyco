...
const ProductHero = ({ product, selectedFinish, currentPrice, mrpPrice, currentSizeLabel, currentFinishName, onAddToCart, onSampleOrder, onVisualizer }) => {
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(
        <svg key={i} className="w-4 h-4 fill-[#FF9900]" viewBox="0 0 20 20">
    ...
    for (let i = 0; i < emptyStars; i++) {
      stars.push(
        <svg key={`empty-${i}`} className="w-4 h-4 fill-[#CCCCCC]" viewBox="0 0 20 20">
          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
        </svg>
      );
    }
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1.5">
                  {/* Amazon-style: number → stars → count */}
                  <span className="text-sm font-medium text-[#007185] hover:text-[#C7511F] cursor-pointer">
                    {product.rating.toFixed(1)}
                  </span>
                  <div className="flex items-center gap-[1px]">
                    {renderStars(product.rating)}
                  </div>
                  <a href="#reviews" className="text-sm text-[#007185] hover:text-[#C7511F] hover:underline ml-0.5">
