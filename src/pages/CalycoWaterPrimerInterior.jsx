...
const interiorPrimerDetail = {
    ...
    recommended_uses: ["Plaster", "Putty surfaces", "Drywall", "Concrete"],
    ...
    technicalSpecs: {
        ...
        recoatTime: "4 hours",
        application_instructions: "Dilute up to 10% with clean water. Apply with brush, roller or spray.",
        shelf_life: "3 Years",
        storage_temp: "Cool, dry place",
        cleanup: "Clean water",
    },
    safety_warnings: {
        hazard_statements: ["May cause mild skin irritation on prolonged contact."],
        precautionary_statements: ["Keep out of reach of children.", "Use in well-ventilated areas.", "Wear protective gloves/eye protection."],
        first_aid: {
...
const CalycoWaterPrimerInterior = () => {
    ...
    const calculateMRP = (sizeLabel) => {
        const mrpData = PRIMER_MRP[normalizedSelectedSheen];
    ...
    useEffect(() => {
        setProduct(interiorPrimerDetail);
        setSelectedSheen(interiorPrimerDetail.defaultFinish || "Matte");

        const finishPricing = (interiorPrimerDetail.priceByFinish || interiorPrimerDetail.price_by_finish || {})["Matte"];
        if (finishPricing && typeof finishPricing === "object") {
            const sizeKeys = Object.keys(finishPricing);
            if (sizeKeys.length > 0) {
                setSelectedSize(sizeKeys[0]);
            } else if (interiorPrimerDetail.packaging && interiorPrimerDetail.packaging.length > 0) {
                setSelectedSize(interiorPrimerDetail.packaging[0]);
            }
        } else if (interiorPrimerDetail.packaging && interiorPrimerDetail.packaging.length > 0) {
            setSelectedSize(interiorPrimerDetail.packaging[0]);
        }

        if (Array.isArray(interiorPrimerDetail.images) && interiorPrimerDetail.images.length > 0) {
            setSelectedImage(interiorPrimerDetail.images[0]);
            setSelectedImageIndex(0);
        } else {
            setSelectedImage(interiorPrimerDetail.image);
            setSelectedImageIndex(0);
        }
        document.title = interiorPrimerDetail.name;
        setLoading(false);
    }, []);
...
export default CalycoWaterPrimerInterior;
