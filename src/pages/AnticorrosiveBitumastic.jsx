...
import { 
  ...
  FiThermometer, FiAlertTriangle, FiAnchor, FiTool 
} from 'react-icons/fi';
...
const bitumasticDetail = {
  ...
  advantages: [
    ...
    "High-build coating for thick protection",
    "Suitable for underground and marine use"
  ],
  application: ["Underground Pipelines", "Storage Tanks", "Structural Steel", "Metal Bridges"],
  recommended_uses: ["Marine Structures", "Industrial Equipment", "Foundations", "Submerged Steel"],
  substrate: ["Ferrous Metal", "Steel", "Iron", "Concrete"],
  coats_required: "2-3 coats (150-200 microns)",
  coverage: "40-50 sq.ft./L per coat",
  technicalSpecs: {
    ...
    full_cure: "7 days",
    application_instructions: "Apply criss-cross pattern. Thin max 10% with mineral spirits. Apply thick uniform coats.",
    shelf_life: "5 Years (Warranty)",
    storage_temp: "Cool, dry place (Away from ignition)",
    cleanup: "Mineral spirits / Xylene",
    preparation_instructions: "Clean to bare metal (SA 2.5). Remove rust/mill scale. Surface must be dry and clean."
  },
  safety_warnings: {
    hazard_statements: [
    ...
    first_aid: {
        ...
        skin_contact: "Wash with soap and water immediately. Remove contaminated clothing.",
        eye_contact: "Rinse cautiously with water for several minutes. Remove contact lenses.",
...
const AnticorrosiveBitumastic = () => {
    // Bitumastic is almost always Black. We force the color selection logic to handle this.
    ...
    const [selectedSheen, setSelectedSheen] = useState("Semi-Gloss");
    ...
    const normalizedSelectedSheen = selectedSheen || product?.defaultFinish || "Semi-Gloss";
    ...
    const calculateMRP = (sizeLabel) => {
        const mrpData = BITUMASTIC_MRP[normalizedSelectedSheen];
    ...
    useEffect(() => {
        setProduct(bitumasticDetail);
        setSelectedSheen(bitumasticDetail.defaultFinish || "Semi-Gloss");

        const finishPricing = (bitumasticDetail.priceByFinish || bitumasticDetail.price_by_finish || {})["Semi-Gloss"];
        if (finishPricing && typeof finishPricing === "object") {
            const sizeKeys = Object.keys(finishPricing);
            if (sizeKeys.length > 0) {
                setSelectedSize(sizeKeys[0]);
            } else if (bitumasticDetail.packaging && bitumasticDetail.packaging.length > 0) {
                setSelectedSize(bitumasticDetail.packaging[0]);
            }
        } else if (bitumasticDetail.packaging && bitumasticDetail.packaging.length > 0) {
            setSelectedSize(bitumasticDetail.packaging[0]);
        }

        if (Array.isArray(bitumasticDetail.images) && bitumasticDetail.images.length > 0) {
            setSelectedImage(bitumasticDetail.images[0]);
            setSelectedImageIndex(0);
        } else {
            setSelectedImage(bitumasticDetail.image);
            setSelectedImageIndex(0);
        }
        document.title = bitumasticDetail.name;
        setLoading(false);
    }, []);
    ...
    const handleAddToCart = () => {
        ...
        setCartPopup({
            ...
            item: {
                name: product.name,
                hex: "#1A1A1A", // Bitumen is black
                colorName: "Bitumen Black",
                colorFamily: "Industrial",
                selectedSheen,
                selectedSize,
                selectedColorType,
                quantity,
            ...
            >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-16 items-start">
                                ...
                                <img
                                    ...
                                    onError={(e) => {
                                        ...
                                        if (!e.target.src.includes("placeholder")) {
                                            e.target.src = "https://placehold.co/600x600/1A1A1A/FFFFFF?text=Bitumastic+Paint";
                                        }
                                    }}
                    ...
                    >
                        ...
                        {Array.isArray(product.features) && product.features.length > 0 && (
                          <div className="my-2">
                            <div className="bg-gradient-to-br from-[#1f2937]/10 to-[#493657]/5 rounded-xl border-2 border-[#1f2937]/20 p-4 sm:p-6 shadow-md">
                              <ul className="space-y-2 sm:space-y-3">
                                {product.features.map((feature, idx) => (
                                  <li key={idx} className="flex items-start gap-3">
                                    <FiCheckCircle className="w-4 h-4 sm:w-5 sm:h-5 text-[#1f2937] mt-0.5 flex-shrink-0" />
                        ...
                        {/* 3 Feature Cards - Bitumastic Specific (Industrial Theme) */}
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 my-3">
                          <div className="bg-[#1f2937] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                            <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-md">
                              <FiShield className="w-6 h-6 sm:w-7 sm:h-7 text-[#1f2937]" />
                            </div>
                            <h4 className="font-bold text-white text-sm sm:text-base mb-1 drop-shadow-sm">Anti-Corrosive</h4>
                          ...
                          <div className="bg-[#2563eb] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                            ...
                            <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">Rubberized</p>
                          </div>
                          <div className="bg-[#4b5563] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                            ...
                            <h4 className="font-bold text-white text-sm sm:text-base mb-1 drop-shadow-sm">Industrial</h4>
                            <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">High Build</p>
                          </div>
                        </div>
                                    ...
                                    {displaySizes.map((size) => (
                                      <button
                                        ...
                                        className={`px-3 sm:px-4 py-2 rounded-lg border transition-all text-sm sm:text-base ${
                                          selectedSize === size
                                            ? "border-[#1f2937] bg-[#1f2937]/10 text-[#493657]"
                            ...
                            <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-md border border-[#493657]/10">
                                <div className="flex flex-col">
                                    <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
                                        <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-[#1f2937] to-[#374151] rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0">
                                    ...
                                    <div className="space-y-3 sm:space-y-4">
                                        {Array.isArray(product.advantages) && product.advantages.length > 0 ? (
                                            product.advantages.map((adv, idx) => (
                                                <div key={idx} className="flex items-start gap-3 p-3 sm:p-4 bg-[#f8fafc] rounded-lg border-l-4 border-[#1f2937]">
                                                    <div className="w-5 h-5 sm:w-6 sm:h-6 bg-[#1f2937] rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                ...
                <div className="mt-8 sm:mt-12 lg:mt-16">
                    <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-[#493657]/10 overflow-hidden">
                        <div className="bg-gradient-to-r from-[#1f2937] via-[#374151] to-[#111827] text-white p-4 sm:p-6 relative overflow-hidden">
                            ...
                            {[
                                ...
                                <div key={idx} className="bg-white p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-0 hover:bg-[#f8fafc] transition-colors">
                                    <div className="flex items-center gap-3">
                                        <spec.icon className="w-4 h-4 sm:w-5 sm:h-5 text-[#1f2937]" />
                ...
                <div className="mt-8 sm:mt-12 lg:mt-16">
                    <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-[#493657]/10 overflow-hidden">
                        <div className="bg-gradient-to-r from-[#1f2937] via-[#374151] to-[#111827] text-white p-4 sm:p-6 text-center relative overflow-hidden">
                            <div className="absolute inset-0 bg-gradient-to-b from-transparent via-white/5 to-black/20"></div>
                    ...
                    <div className="bg-gradient-to-br from-[#f8fafc] to-[#f1f5f9] rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 shadow-lg border border-[#493657]/10">
                        ...
                        <div className="grid grid-cols-1 gap-6 sm:gap-8 md:grid-cols-2">
                            ...
                            <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg border border-[#493657]/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group">
                                ...
                                <a
                                    href="/Assets/docs/html-templates/calyco-bitumastic-tds.html"
                                    download
                ...
                {/* ENHANCED Features Section - Adapted for Bitumastic Paint */}
                <motion.div
                    className="mt-8 sm:mt-12 lg:mt-16"
                    variants={itemVariants}
                >
                    <div className="relative rounded-2xl sm:rounded-3xl bg-gradient-to-br from-[#111827]/5 via-white to-[#1f2937]/5 p-6 sm:p-8 md:p-12 shadow-2xl overflow-hidden border border-[#493657]/10">
                        {/* Premium Background Effects - Industrial Tones */}
                        <div className="pointer-events-none absolute -top-24 -right-12 w-96 h-96 bg-gradient-to-br from-[#1f2937]/20 to-[#493657]/10 blur-3xl rounded-full" />
                        <div className="pointer-events-none absolute -bottom-32 -left-10 w-80 h-80 bg-gradient-to-tr from-[#493657]/15 to-[#1f2937]/10 blur-3xl rounded-full" />
                        <div className="pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-gradient-to-r from-[#374151]/10 to-[#111827]/10 blur-2xl rounded-full" />

                        <div className="relative z-10">
                            <h2 className="text-3xl sm:text-4xl font-bold text-[#493657] mb-3 sm:mb-4 text-center">Industrial Protection</h2>
                            <p className="text-center text-[#493657]/70 mb-8 sm:mb-12 text-base sm:text-lg">Heavy-duty coating for harsh environments</p>
                            ...
                            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-4 sm:gap-8">
                                ...
                                <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#1f2937] transition-all duration-500 hover:-translate-y-2">
                                    <div className="w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-[#1f2937] via-[#374151] to-[#4b5563] rounded-xl sm:rounded-2xl flex items-center justify-center mb-4 sm:mb-6 group-hover:scale-110 transition-transform duration-300 shadow-lg">
                                    ...
                                    <p className="text-sm text-[#493657]/70 leading-relaxed">Resists attack from acids, alkalis, and industrial chemicals.</p>
                                    <div className="mt-3 sm:mt-4 h-1 w-10 sm:w-12 bg-gradient-to-r from-[#1f2937] to-[#1f2937]/40 rounded-full group-hover:w-14 sm:group-hover:w-16 transition-all duration-300"></div>
                                </div>

                                {/* Feature 2: High Build */}
                                <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#1f2937] transition-all duration-500 hover:-translate-y-2">
                                    <div className="w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-[#2563eb] via-[#3b82f6] to-[#60a5fa] rounded-xl sm:rounded-2xl flex items-center justify-center mb-4 sm:mb-6 group-hover:scale-110 transition-transform duration-300 shadow-lg">
                                        <FiLayers className="w-7 h-7 sm:w-8 sm:h-8 text-white drop-shadow-sm" />
                                    </div>
                                    <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">High Build</h3>
                                    <p className="text-sm text-[#493657]/70 leading-relaxed">Thick application creates a durable barrier against moisture penetration.</p>
                                    <div className="mt-3 sm:mt-4 h-1 w-10 sm:w-12 bg-gradient-to-r from-[#2563eb] to-[#2563eb]/40 rounded-full group-hover:w-14 sm:group-hover:w-16 transition-all duration-300"></div>
                                </div>

                                {/* Feature 3: Thermal Flex */}
                                <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#1f2937] transition-all duration-500 hover:-translate-y-2">
                                    ...
                                    <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">Flexible</h3>
                                    <p className="text-sm text-[#493657]/70 leading-relaxed">Bitumen base allows expansion and contraction without cracking.</p>
