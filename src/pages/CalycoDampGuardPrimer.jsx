...
const dampGuardDetail = {
    ...
    advantages: [
        "Long-term waterproofing and damp resistance",
        "Bridges hairline cracks to prevent failures",
        "Breathable film allows moisture vapour escape",
        "Great adhesion across interior/exterior surfaces",
    ],
    application: ["Bathrooms", "Kitchens", "Basements", "Roof slabs", "Exterior walls"],
    recommended_uses: ["Bathrooms", "Kitchens", "Basements", "Roof slabs", "Exterior walls"],
    substrate: ["Cement plaster", "Concrete", "Brick", "Masonry", "Fibre cement board"],
    coats_required: "2-3 coats (as needed for waterproofing)",
    coverage: "90-110 sq.ft./L per coat",
    technicalSpecs: {
        product_code: "CAL-PRI-WP-005",
        base_type: "Water-based acrylic elastomeric",
        voc_content: "< 50 g/L",
        dryingTime: "45 minutes (surface dry)",
        recoatTime: "5 hours",
        application_instructions: "Mix well. First coat diluted 10-15% with clean water; subsequent coats undiluted. Apply by brush or roller perpendicular between coats. Allow 5-6 hours between coats.",
        shelf_life: "3 Years",
        storage_temp: "Cool, dry place; do not freeze",
        cleanup: "Clean water",
    },
    safety_warnings: {
        hazard_statements: ["May cause mild skin/eye irritation on contact."],
        precautionary_statements: ["Keep out of reach of children.", "Use in well-ventilated areas.", "Avoid contact with eyes.", "Do not freeze.", "Store in a cool, dry place."],
        first_aid: {
...
const CalycoDampGuardPrimer = () => {
    ...
    const calculateMRP = (sizeLabel) => {
        const mrpData = DAMP_GUARD_MRP[normalizedSelectedSheen];
    ...
    useEffect(() => {
        setProduct(dampGuardDetail);
        setSelectedSheen(dampGuardDetail.defaultFinish || "Matte");

        const finishPricing = (dampGuardDetail.priceByFinish || dampGuardDetail.price_by_finish || {})["Matte"];
        if (finishPricing && typeof finishPricing === "object") {
            const sizeKeys = Object.keys(finishPricing);
            if (sizeKeys.length > 0) {
                setSelectedSize(sizeKeys[0]);
            } else if (dampGuardDetail.packaging && dampGuardDetail.packaging.length > 0) {
                setSelectedSize(dampGuardDetail.packaging[0]);
            }
        } else if (dampGuardDetail.packaging && dampGuardDetail.packaging.length > 0) {
            setSelectedSize(dampGuardDetail.packaging[0]);
        }

        if (Array.isArray(dampGuardDetail.images) && dampGuardDetail.images.length > 0) {
            setSelectedImage(dampGuardDetail.images[0]);
            setSelectedImageIndex(0);
        } else {
            setSelectedImage(dampGuardDetail.image);
            setSelectedImageIndex(0);
        }
        document.title = dampGuardDetail.name;
        setLoading(false);
    }, []);
                            ...
                            <div className="grid grid-cols-1 gap-6 sm:gap-8 md:grid-cols-2">
                                ...
                                <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg border border-[#493657]/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2 mt-3">
                                        <a href="/Assets/docs/html-templates/calyco-damp-guard-primer-tds.html" download className="flex-1 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiDownload className="w-4 h-4 sm:w-5 sm:h-5" />
                                            Download TDS
                                        </a>
                                        <a href="/Assets/docs/html-templates/calyco-damp-guard-primer-tds.html" target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                ...
                                <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg border border-[#493657]/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2 mt-3">
                                        <a href="/Assets/docs/html-templates/calyco-damp-guard-primer-sds.html" download className="flex-1 bg-gradient-to-r from-[#f59e0b] to-[#d97706] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiDownload className="w-4 h-4 sm:w-5 sm:h-5" />
                                            Download SDS
                                        </a>
                                        <a href="/Assets/docs/html-templates/calyco-damp-guard-primer-sds.html" target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-r from-[#f59e0b] to-[#d97706] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiExternalLink className="w-4 h-4 sm:w-5 sm:h-5" />
                                            View SDS
                                        </a>
                                    </div>
...
export default CalycoDampGuardPrimer;
