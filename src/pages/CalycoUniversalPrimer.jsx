...
const universalPrimerDetail = {
    ...
    advantages: [
        "Excellent adhesion across diverse substrates",
        "High opacity reduces number of topcoats",
        "Corrosion resistance on ferrous metals",
        "Quick drying helps speed project timelines",
    ],
    application: ["Metal gates & railings", "Wooden furniture & doors", "Masonry walls", "GI sheet roofing", "Fibre cement boards"],
    recommended_uses: ["Metal gates & railings", "Wooden furniture & doors", "Masonry walls", "GI sheet roofing", "Fibre cement boards"],
    substrate: ["Metal", "Wood", "Masonry", "GI sheet", "Fibre cement board"],
    coats_required: "1-2 coats (2 on metal for rust protection)",
    ...
    technicalSpecs: {
        ...
        recoatTime: "8 hours",
        application_instructions: "Stir well. Thin with mineral turpentine if required (max 5-10%). Apply 1-2 uniform coats by brush, roller, or spray. For metal, apply 2 coats for better rust protection.",
        shelf_life: "3 Years",
        storage_temp: "Cool, dry, well-ventilated area away from flame",
...
const CalycoUniversalPrimer = () => {
    ...
    useEffect(() => {
        setProduct(universalPrimerDetail);
        setSelectedSheen(universalPrimerDetail.defaultFinish || "Matte");

        const finishPricing = (universalPrimerDetail.priceByFinish || universalPrimerDetail.price_by_finish || {})["Matte"];
        if (finishPricing && typeof finishPricing === "object") {
            const sizeKeys = Object.keys(finishPricing);
            if (sizeKeys.length > 0) {
                setSelectedSize(sizeKeys[0]);
            } else if (universalPrimerDetail.packaging && universalPrimerDetail.packaging.length > 0) {
                setSelectedSize(universalPrimerDetail.packaging[0]);
            }
        } else if (universalPrimerDetail.packaging && universalPrimerDetail.packaging.length > 0) {
            setSelectedSize(universalPrimerDetail.packaging[0]);
        }

        if (Array.isArray(universalPrimerDetail.images) && universalPrimerDetail.images.length > 0) {
            setSelectedImage(universalPrimerDetail.images[0]);
            setSelectedImageIndex(0);
        } else {
            setSelectedImage(universalPrimerDetail.image);
            setSelectedImageIndex(0);
        }
        document.title = universalPrimerDetail.name;
        setLoading(false);
    }, []);
                            ...
                            <div className="grid grid-cols-1 gap-6 sm:gap-8 md:grid-cols-2">
                                ...
                                <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg border border-[#493657]/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2 mt-3">
                                        <a href="/Assets/docs/html-templates/calyco-universal-primer-tds.html" download className="flex-1 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiDownload className="w-4 h-4 sm:w-5 sm:h-5" />
                                            Download TDS
                                        </a>
                                        <a href="/Assets/docs/html-templates/calyco-universal-primer-tds.html" target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                ...
                                <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg border border-[#493657]/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2 mt-3">
                                        <a href="/Assets/docs/html-templates/calyco-universal-primer-sds.html" download className="flex-1 bg-gradient-to-r from-[#f59e0b] to-[#d97706] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiDownload className="w-4 h-4 sm:w-5 sm:h-5" />
                                            Download SDS
                                        </a>
                                        <a href="/Assets/docs/html-templates/calyco-universal-primer-sds.html" target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-r from-[#f59e0b] to-[#d97706] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiExternalLink className="w-4 h-4 sm:w-5 sm:h-5" />
                                            View SDS
                                        </a>
                                    </div>
...
export default CalycoUniversalPrimer;
