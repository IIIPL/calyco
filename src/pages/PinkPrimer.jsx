...
import { FiTag, FiList, FiCheckCircle, FiDroplet, FiClipboard, FiLayers, FiBox, FiPackage, FiShield, FiArchive, FiAlertCircle, FiInfo, FiCalendar, FiHeart, FiChevronLeft, FiChevronRight, FiWind, FiDownload, FiClock, FiDisc, FiAlertTriangle, FiExternalLink } from 'react-icons/fi';
import { useCart } from "../context/CartContext"; // Ensure this path is correct for your project
import CartPopup from "../components/CartPopup"; // Ensure this path is correct
import RatingStars from "../components/RatingStars"; // Ensure this path is correct
import ReviewsSection from "../components/ReviewsSection"; // Ensure this path is correct
...
const pinkPrimerDetail = {
    ...
    advantages: [
        "Excellent knot and stain-blocking properties",
        "Superior adhesion to wood substrates",
        "Seals porous wood effectively",
        "Fast drying for quick project completion",
        "Compatible with all topcoat finishes"
    ],
    application: ["Doors & Window Frames", "Wooden Furniture", "Plywood & MDF", "New & Bare Wood"],
    recommended_uses: ["Interior Wood", "Exterior Wood", "Softwood", "Hardwood"],
    substrate: ["Wood", "Plywood", "MDF", "Timber"],
    coats_required: "1-2 coats",
    coverage: "100-120 sq.ft./L",
    technicalSpecs: {
        product_code: "CAL-PRM-001",
        base_type: "Alkyd Resin",
        voc_content: "≤ 200 g/L",
        dryingTime: "1-2 hours (Touch Dry)",
        recoatTime: "4-6 hours",
        full_cure: "24 hours",
        application_instructions: "Stir thoroughly. Thin max 10-15% with mineral spirits if needed. Apply with brush, roller or spray.",
        shelf_life: "3 Years",
        storage_temp: "Cool, dry place away from direct sunlight",
        cleanup: "Mineral spirits / Turpentine",
        preparation_instructions: "Sand with 120-180 grit. Ensure surface is clean, dry (<15% moisture), and free from grease/wax."
    },
    safety_warnings: {
        hazard_statements: [
        ...
        first_aid: {
            inhalation: "Move to fresh air. Keep warm and at rest. Avoid breathing vapours or mist.",
            skin_contact: "Remove contaminated clothing. Rinse immediately with plenty of water.",
...
const PinkPrimer = () => {
    // Logic for colors - Standard Pink Primer implies a pinkish hue
    ...
    const calculateMRP = (sizeLabel) => {
        const mrpData = PINK_PRIMER_MRP[normalizedSelectedSheen];
    ...
    useEffect(() => {
        ...
        if (finishPricing && typeof finishPricing === "object") {
            ...
            } else if (pinkPrimerDetail.packaging && pinkPrimerDetail.packaging.length > 0) {
                setSelectedSize(pinkPrimerDetail.packaging[0]);
            }
        } else if (pinkPrimerDetail.packaging && pinkPrimerDetail.packaging.length > 0) {
            setSelectedSize(pinkPrimerDetail.packaging[0]);
        }

        if (Array.isArray(pinkPrimerDetail.images) && pinkPrimerDetail.images.length > 0) {
            setSelectedImage(pinkPrimerDetail.images[0]);
            setSelectedImageIndex(0);
        } else {
            setSelectedImage(pinkPrimerDetail.image);
            setSelectedImageIndex(0);
        }
        document.title = pinkPrimerDetail.name;
        setLoading(false);
    }, []);
    ...
    const handleAddToCart = () => {
        ...
        setCartPopup({
            ...
            item: {
                name: product.name,
                hex: "#FFC0CB", // Defaulting to a pinkish hex for Pink Primer
                colorName: "Pink Base",
                colorFamily: "Primer",
                selectedSheen,
                selectedSize,
                ...
                >
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-16 items-start">
                                    ...
                                    <div className="relative bg-white rounded-xl sm:rounded-2xl p-2 sm:p-4">
                                        <img
                                            ...
                                            onError={(e) => {
                                                ...
                                                // Fallback to a place holder if real image fails
                                                if (!e.target.src.includes("placeholder")) {
                                                    e.target.src = "https://placehold.co/600x600?text=Pink+Primer";
                                                }
                                            }}
                        ...
                        >
                            ...
                            {/* 3 Feature Cards - Pink Primer Specific */}
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 my-3">
                                <div className="bg-[#00BCD4] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                                    <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-md">
                                        <FiDisc className="w-6 h-6 sm:w-7 sm:h-7 text-[#00BCD4]" />
                                    </div>
                                    <h4 className="font-bold text-white text-sm sm:text-base mb-1 drop-shadow-sm">Seals Knots</h4>
                                    <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">Blocks tannins</p>
                                </div>
                                <div className="bg-[#FF9500] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                                    <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-md">
                                        <FiClock className="w-6 h-6 sm:w-7 sm:h-7 text-[#FF9500]" />
                                    </div>
                                    <h4 className="font-bold text-white text-sm sm:text-base mb-1 drop-shadow-sm">Fast Drying</h4>
                                    <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">Recoat in 4h</p>
                                </div>
                                <div className="bg-[#34C759] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                                    ...
                                    <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">Smooth base</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4 sm:space-y-6">
                                ...
                                {/* Color Mixing Option (Disabled for Primer generally, but kept structure) */}
                                {ALLOW_COLOR_MIXING && (
                                    <div className="mb-4">
                                        ...
                                        {/* ... Color mixing buttons omitted as ALLOW_COLOR_MIXING is false ... */}
                                    </div>
                                )}
                        ...
                        <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-[#493657]/10 overflow-hidden">
                            ...
                            <div className="grid grid-cols-1 gap-px bg-[#e2e8f0]">
                                {[
                                    ...
                                    { label: "Base Type", value: product.technicalSpecs?.base_type || 'Alkyd', icon: FiBox },
                            ...
                            <div className="grid grid-cols-1 gap-6 sm:gap-8 md:grid-cols-2">
                                ...
                                <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg border border-[#493657]/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2 mt-3">
                                        <a href="/Assets/docs/html-templates/calyco-pink-primer-tds.html" download className="flex-1 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiDownload className="w-4 h-4 sm:w-5 sm:h-5" />
                                            Download TDS
                                        </a>
                                        <a href="/Assets/docs/html-templates/calyco-pink-primer-tds.html" target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                ...
                                <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg border border-[#493657]/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2 mt-3">
                                        <a href="/Assets/docs/html-templates/calyco-pink-primer-sds.html" download className="flex-1 bg-gradient-to-r from-[#f59e0b] to-[#d97706] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiDownload className="w-4 h-4 sm:w-5 sm:h-5" />
                                            Download SDS
                                        </a>
                                        <a href="/Assets/docs/html-templates/calyco-pink-primer-sds.html" target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-r from-[#f59e0b] to-[#d97706] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiExternalLink className="w-4 h-4 sm:w-5 sm:h-5" />
                                            View SDS
                                        </a>
                                    </div>
                    ...
                    {/* ENHANCED Features Section - Adapted for Pink Primer */}
                    <motion.div
                        className="mt-8 sm:mt-12 lg:mt-16"
                        variants={itemVariants}
                    >
                        <div className="relative rounded-2xl sm:rounded-3xl bg-gradient-to-br from-[#493657]/5 via-white to-[#F0C85A]/5 p-6 sm:p-8 md:p-12 shadow-2xl overflow-hidden border border-[#493657]/10">
                            ...
                            <div className="relative z-10">
                                <h2 className="text-3xl sm:text-4xl font-bold text-[#493657] mb-3 sm:mb-4 text-center">Premium Wood Care</h2>
                                <p className="text-center text-[#493657]/70 mb-8 sm:mb-12 text-base sm:text-lg">Specialized protection for your valuable wood surfaces</p>

                                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-4 sm:gap-8">
                                    {/* Feature 1: Knot Sealing */}
                                    <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                        <div className="w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-[#FFC107] via-[#FFD54F] to-[#FFEB3B] rounded-xl sm:rounded-2xl flex items-center justify-center mb-4 sm:mb-6 group-hover:scale-110 transition-transform duration-300 shadow-lg">
                                            <FiDisc className="w-7 h-7 sm:w-8 sm:h-8 text-white drop-shadow-sm" />
                                        </div>
                                        <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">Knot Sealing</h3>
                                        <p className="text-sm text-[#493657]/70 leading-relaxed">Locks in resins and tannins, preventing bleed-through and stains on the topcoat.</p>
                                    ...
                                    <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                        ...
                                        <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">Deep Adhesion</h3>
                                        <p className="text-sm text-[#493657]/70 leading-relaxed">Alkyd formula penetrates deep into wood grain for a permanent bond.</p>
                                    ...
                                    <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                        ...
                                        <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">Fast Drying</h3>
                                        <p className="text-sm text-[#493657]/70 leading-relaxed">Touch dry in 1-2 hours, allowing you to complete your painting project faster.</p>
