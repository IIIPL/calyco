...
// --- DATA DEFINITION FOR ENAMEL (Embedded for self-containment) ---
const amrellaEnamelDetail = {
    id: "calyco-amrella-enamel",
    name: "Dura-Shield Enamel",
    slug: "calyco-amrella-enamel",
    image: "/Assets/Product Images/Dura-Shield Enamel/dura-shield-enamel.webp",
    images: ["/Assets/Product Images/Dura-Shield Enamel/dura-shield-enamel.webp"],
    ...
    tagline: "Brilliant finish that stands the test of time.",
    ...
    advantages: [
        ...
        "Weather and UV durability",
        "Rust protection on metal substrates",
        "Quick drying, easy maintenance",
    ],
    application: ["Interior & exterior wood", "Metal surfaces", "Masonry", "Plaster"],
    recommended_uses: ["Interior & exterior wood", "Metal surfaces", "Masonry", "Plaster"],
    substrate: ["Wood", "Metal", "Masonry", "Plaster"],
    coats_required: "2 coats",
    coverage: "100-140 sq.ft./L",
    technicalSpecs: {
        product_code: "CAL-ENL-HG-001",
        base_type: "Modified alkyd resin (solvent-based)",
        voc_content: "≤ 300 g/L",
        dryingTime: "1-2 hours (surface dry)",
        recoatTime: "6-8 hours",
        application_instructions: "Stir well. Thin up to 5-10% with synthetic thinner if required. Apply evenly by brush/roller; allow 6-8 hours between coats. For spray, adjust thinning as needed.",
        shelf_life: "3 Years",
        storage_temp: "Cool, dry place away from heat and flame",
        cleanup: "Mineral turpentine / synthetic thinner",
    },
    safety_warnings: {
        hazard_statements: ["Flammable liquid and vapour.", "May cause skin/eye irritation."],
...
const AmrellaEnamel = () => {
    ...
    const calculateMRP = (sizeLabel) => {
        const mrpData = ENAMEL_MRP[normalizedSelectedSheen];
    ...
    useEffect(() => {
        setProduct(amrellaEnamelDetail);
        setSelectedSheen(amrellaEnamelDetail.defaultFinish || "High Gloss");

        const finishPricing = (amrellaEnamelDetail.priceByFinish || amrellaEnamelDetail.price_by_finish || {})["High Gloss"];
        if (finishPricing && typeof finishPricing === "object") {
            const sizeKeys = Object.keys(finishPricing);
            if (sizeKeys.length > 0) {
                setSelectedSize(sizeKeys[0]);
            } else if (amrellaEnamelDetail.packaging && amrellaEnamelDetail.packaging.length > 0) {
                setSelectedSize(amrellaEnamelDetail.packaging[0]);
            }
        } else if (amrellaEnamelDetail.packaging && amrellaEnamelDetail.packaging.length > 0) {
            setSelectedSize(amrellaEnamelDetail.packaging[0]);
        }

        if (Array.isArray(amrellaEnamelDetail.images) && amrellaEnamelDetail.images.length > 0) {
            setSelectedImage(amrellaEnamelDetail.images[0]);
            setSelectedImageIndex(0);
        } else {
            setSelectedImage(amrellaEnamelDetail.image);
            setSelectedImageIndex(0);
        }
        document.title = amrellaEnamelDetail.name;
        setLoading(false);
    }, []);
                ...
                >
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-16 items-start">
                        ...
                        >
                            ...
                            {/* 3 Feature Cards - Enamel Specific */}
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 my-3">
                                <div className="bg-[#FF9500] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                                    <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-md">
                                        <FiStar className="w-6 h-6 sm:w-7 sm:h-7 text-[#FF9500]" />
                                    </div>
                                    <h4 className="font-bold text-white text-sm sm:text-base mb-1 drop-shadow-sm">High Gloss</h4>
                                    <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">Mirror-like finish</p>
                                </div>
                                <div className="bg-[#2563eb] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                                    <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-md">
                                        <FiShield className="w-6 h-6 sm:w-7 sm:h-7 text-[#2563eb]" />
                                    </div>
                                ...
                                <div className="bg-[#34C759] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                                    ...
                                    <h4 className="font-bold text-white text-sm sm:text-base mb-1 drop-shadow-sm">Anti-Rust</h4>
                                    <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">Metal protection</p>
                                </div>
                            </div>
                        ...
                        <div className="relative rounded-2xl sm:rounded-3xl bg-gradient-to-br from-[#493657]/5 via-white to-[#F0C85A]/5 p-6 sm:p-8 md:p-12 shadow-2xl overflow-hidden border border-[#493657]/10">
                            ...
                            <div className="relative z-10">
                                ...
                                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-4 sm:gap-8">
                                    {/* Feature 1: Mirror-like Finish (Gold) */}
                                    <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                        ...
                                        <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">Mirror-like Finish</h3>
                                        <p className="text-sm text-[#493657]/70 leading-relaxed">Delivers a brilliant, high-gloss shine that enhances the beauty of any surface.</p>
                                    ...
                                    {/* Feature 2: Superior Adhesion (Purple) */}
                                    <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                        ...
                                        <p className="text-sm text-[#493657]/70 leading-relaxed">Strong bonding capabilities on wood, metal, and masonry surfaces.</p>
                                    ...
                                    {/* Feature 3: Rust Protection (Green) */}
                                    <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                        ...
                                        <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">Rust Protection</h3>
                                        <p className="text-sm text-[#493657]/70 leading-relaxed">Forms a protective barrier that prevents rust formation on metal surfaces.</p>
                                    ...
                                    {/* Feature 4: Weather Resistant (Cyan) */}
                                    <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                        ...
                                        <p className="text-sm text-[#493657]/70 leading-relaxed">Withstands harsh weather conditions and UV radiation for long-lasting color.</p>
