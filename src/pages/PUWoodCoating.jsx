...
// --- DATA DEFINITION FOR WOOD COATING (Embedded for self-containment) ---
const puWoodCoatingDetail = {
    id: "calyco-pu-wood-coating",
    ...
    advantages: [
        "Accentuates natural wood grain with clarity",
        "Superior scratch, stain, and heat resistance",
        "Non-yellowing durability over time",
        "Anti-fungal properties safeguard wood",
    ],
    application: ["Wood furniture", "Doors & windows", "MDF surfaces", "Veneer", "Wooden flooring"],
    recommended_uses: ["Wood furniture", "Doors & windows", "MDF surfaces", "Veneer", "Wooden flooring"],
    ...
    technicalSpecs: {
        ...
        recoatTime: "6 hours",
        application_instructions: "Mix base and hardener per specified ratio (e.g., 2:1 or 4:1). Stir 2-3 minutes; allow 5-10 minutes induction. Pot life 4-6 hours. Apply 2-3 coats by brush/roller/spray; thin with PU thinner 5-10% if needed; sand lightly between coats.",
        shelf_life: "3 Years",
        storage_temp: "Cool, dry place; keep components sealed",
        cleanup: "PU thinner",
    },
    safety_warnings: {
        hazard_statements: ["Flammable and harmful vapours.", "May cause skin/eye irritation."],
        precautionary_statements: [
            "Keep away from heat, sparks, and open flames.",
            "Use only in well-ventilated areas and wear respirator when spraying.",
            "Wear protective gloves/eye protection.",
            "Keep out of reach of children.",
            "Use mixed product within pot life.",
        ],
        first_aid: {
...
const PUWoodCoating = () => {
    ...
    const calculateMRP = (sizeLabel) => {
        const mrpData = PU_WOOD_MRP[normalizedSelectedSheen];
    ...
    useEffect(() => {
        setProduct(puWoodCoatingDetail);
        setSelectedSheen(puWoodCoatingDetail.defaultFinish || "High Gloss");

        const finishPricing = (puWoodCoatingDetail.priceByFinish || puWoodCoatingDetail.price_by_finish || {})["High Gloss"];
        if (finishPricing && typeof finishPricing === "object") {
            const sizeKeys = Object.keys(finishPricing);
            if (sizeKeys.length > 0) {
                setSelectedSize(sizeKeys[0]);
            } else if (puWoodCoatingDetail.packaging && puWoodCoatingDetail.packaging.length > 0) {
                setSelectedSize(puWoodCoatingDetail.packaging[0]);
            }
        } else if (puWoodCoatingDetail.packaging && puWoodCoatingDetail.packaging.length > 0) {
            setSelectedSize(puWoodCoatingDetail.packaging[0]);
        }

        if (Array.isArray(puWoodCoatingDetail.images) && puWoodCoatingDetail.images.length > 0) {
            setSelectedImage(puWoodCoatingDetail.images[0]);
            setSelectedImageIndex(0);
        } else {
            setSelectedImage(puWoodCoatingDetail.image);
            setSelectedImageIndex(0);
        }
        document.title = puWoodCoatingDetail.name;
        setLoading(false);
    }, []);
                            ...
                            <div className="grid grid-cols-1 gap-6 sm:gap-8 md:grid-cols-2">
                                ...
                                <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg border border-[#493657]/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2 mt-3">
                                        <a href="/Assets/docs/html-templates/calyco-pu-wood-coating-tds.html" download className="flex-1 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiDownload className="w-4 h-4 sm:w-5 sm:h-5" />
                                            Download TDS
                                        </a>
                                        <a href="/Assets/docs/html-templates/calyco-pu-wood-coating-tds.html" target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                ...
                                <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg border border-[#493657]/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2 mt-3">
                                        <a href="/Assets/docs/html-templates/calyco-pu-wood-coating-sds.html" download className="flex-1 bg-gradient-to-r from-[#f59e0b] to-[#d97706] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiDownload className="w-4 h-4 sm:w-5 sm:h-5" />
                                            Download SDS
                                        </a>
                                        <a href="/Assets/docs/html-templates/calyco-pu-wood-coating-sds.html" target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-r from-[#f59e0b] to-[#d97706] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiExternalLink className="w-4 h-4 sm:w-5 sm:h-5" />
                                            View SDS
                                        </a>
                                    </div>
