...
import { 
  ...
  FiActivity, FiAlertTriangle, FiHexagon, FiZap 
} from 'react-icons/fi';
...
const epoxyPaintDetail = {
  ...
  slug: "epoxy-paint",
  image: "/Assets/Product Images/Epoxy Paint/calyco-epoxy-paint.webp", 
  images: ["/Assets/Product Images/Epoxy Paint/calyco-epoxy-paint.webp"],
  ...
  tagline: "Industrial-strength protection for demanding environments.",
  ...
  application: ["Industrial Floors", "Warehouse Floors", "Chemical Storage", "Parking Garages"],
  recommended_uses: ["Storage Tanks", "Processing Equipment", "Showrooms", "Workshops"],
  substrate: ["Concrete", "Metal", "Stone", "Masonry"],
  coats_required: "2-3 coats (200-300 microns DFT)",
  coverage: "80-120 sq.ft./L at 100 microns",
  technicalSpecs: {
    ...
    full_cure: "7 days",
    pot_life: "30-45 minutes at 25°C",
    application_instructions: "Mix Part A & B (2:1 or 3:1). Stir 3-5 mins. Induction 10-15 mins. Use within pot life.",
    shelf_life: "5 Years (Warranty)",
    storage_temp: "Cool, dry place (Store components separately)",
    cleanup: "Epoxy Thinner",
    preparation_instructions: "Concrete: Shot blast/grind. Moisture <6%. Repair cracks. Surface must be clean/dry."
  },
  safety_warnings: {
    hazard_statements: [
      "DANGER: HARMFUL if swallowed or inhaled.",
      "Causes skin irritation and may cause allergic skin reaction (Sensitization).",
      "Causes serious eye damage."
    ],
    precautionary_statements: [
      "Wear protective gloves, eye protection, and respirator.",
      "Use only in well-ventilated areas.",
      "Avoid breathing vapours/mist.",
      "Do not smoke during application."
    ],
    first_aid: {
        inhalation: "Move to fresh air. Seek medical attention if symptoms persist.",
        skin_contact: "Wash immediately with soap and water. Remove contaminated clothing.",
        eye_contact: "Rinse cautiously with water for several minutes. Get medical advice/attention.",
...
const EpoxyPaint = () => {
    // Logic for colors - Epoxy usually defaults to "Industrial Grey" or similar
    ...
    const normalizedSelectedSheen = selectedSheen || product?.defaultFinish || "High Gloss";
    ...
    const calculateMRP = (sizeLabel) => {
        const mrpData = EPOXY_MRP[normalizedSelectedSheen];
    ...
    useEffect(() => {
        setProduct(epoxyPaintDetail);
        setSelectedSheen(epoxyPaintDetail.defaultFinish || "High Gloss");

        const finishPricing = (epoxyPaintDetail.priceByFinish || epoxyPaintDetail.price_by_finish || {})["High Gloss"];
        if (finishPricing && typeof finishPricing === "object") {
            const sizeKeys = Object.keys(finishPricing);
            if (sizeKeys.length > 0) {
                setSelectedSize(sizeKeys[0]);
            } else if (epoxyPaintDetail.packaging && epoxyPaintDetail.packaging.length > 0) {
                setSelectedSize(epoxyPaintDetail.packaging[0]);
            }
        } else if (epoxyPaintDetail.packaging && epoxyPaintDetail.packaging.length > 0) {
            setSelectedSize(epoxyPaintDetail.packaging[0]);
        }

        if (Array.isArray(epoxyPaintDetail.images) && epoxyPaintDetail.images.length > 0) {
            setSelectedImage(epoxyPaintDetail.images[0]);
            setSelectedImageIndex(0);
        } else {
            setSelectedImage(epoxyPaintDetail.image);
            setSelectedImageIndex(0);
        }
        document.title = epoxyPaintDetail.name;
        setLoading(false);
    }, []);
            ...
            >
                                ...
                                <img
                                    ...
                                    onError={(e) => {
                                        ...
                                        if (!e.target.src.includes("placeholder")) {
                                            e.target.src = "https://placehold.co/600x600/9CA3AF/FFFFFF?text=Epoxy+Paint";
                                        }
                                    }}
                                    ...
                                    {product.images.map((_, idx) => (
                                        <button
                                            ...
                                            className={`w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full transition-all duration-200 ${
                                                idx === selectedImageIndex ? 'bg-[#06b6d4] w-6 sm:w-8' : 'bg-[#493657]/20 hover:bg-[#493657]/40'
                                            }`}
                    ...
                    >
                        ...
                        {totalReviews > 0 && (
                          <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
                            ...
                            <span className="text-sm sm:text-base text-[#493657] font-medium cursor-pointer hover:text-[#06b6d4]" onClick={scrollToReviews}>
                        ...
                        {/* Pricing */}
                        <div>
                            <div className="inline-block bg-[#0891b2] text-white px-3 py-1 text-sm font-semibold rounded mb-2">
                                2-Component System
                            </div>
                            {displayMRPValue ? (
                        ...
                        {Array.isArray(product.features) && product.features.length > 0 && (
                          <div className="my-2">
                            <div className="bg-gradient-to-br from-[#06b6d4]/10 to-[#493657]/5 rounded-xl border-2 border-[#06b6d4]/20 p-4 sm:p-6 shadow-md">
                              <ul className="space-y-2 sm:space-y-3">
                                {product.features.map((feature, idx) => (
                                  <li key={idx} className="flex items-start gap-3">
                                    <FiCheckCircle className="w-4 h-4 sm:w-5 sm:h-5 text-[#0891b2] mt-0.5 flex-shrink-0" />
                        ...
                        {/* 3 Feature Cards - Epoxy Specific (Industrial/Chemical Theme) */}
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 my-3">
                          <div className="bg-[#0891b2] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                            <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-md">
                              <FiHexagon className="w-6 h-6 sm:w-7 sm:h-7 text-[#0891b2]" />
                            </div>
                            <h4 className="font-bold text-white text-sm sm:text-base mb-1 drop-shadow-sm">High Strength</h4>
                            <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">Heavy Duty</p>
                          </div>
                          <div className="bg-[#4f46e5] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                            <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-md">
                              <FiShield className="w-6 h-6 sm:w-7 sm:h-7 text-[#4f46e5]" />
                            </div>
                            <h4 className="font-bold text-white text-sm sm:text-base mb-1 drop-shadow-sm">Chemical Safe</h4>
                            <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">Acid Resistant</p>
                          </div>
                          <div className="bg-[#059669] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                            <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-md">
                              <FiDroplet className="w-6 h-6 sm:w-7 sm:h-7 text-[#059669]" />
                            </div>
                            <h4 className="font-bold text-white text-sm sm:text-base mb-1 drop-shadow-sm">Waterproof</h4>
                            <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">Impermeable</p>
                          </div>
                        </div>
                        ...
                        <div className="space-y-4 sm:space-y-6">
                            ...
                            {ALLOW_COLOR_MIXING && (
                              <div className="mb-4">
                                <h3 className="font-semibold text-[#493657] mb-2 flex flex-col sm:flex-row sm:items-center gap-2 text-sm sm:text-base">
                                  Standard Colors
                                  <span className="text-xs font-normal text-[#493657]/60 bg-[#06b6d4]/10 px-2 py-0.5 rounded-full w-fit">
                                    Industrial Shades
                                  </span>
                                </h3>
                                <div className="flex flex-wrap gap-2">
                                  {/* Dummy buttons to simulate color options since no color data provided for epoxy */}
                                   <button type="button" className={`px-3 py-2 rounded-lg border border-[#06b6d4] bg-[#06b6d4]/10 text-[#493657] text-sm`}>Standard Grey</button>
                                    ...
                                    {displaySizes.map((size) => (
                                      <button
                                        ...
                                        className={`px-3 sm:px-4 py-2 rounded-lg border transition-all text-sm sm:text-base ${
                                          selectedSize === size
                                            ? "border-[#06b6d4] bg-[#06b6d4]/10 text-[#493657]"
                            ...
                            <div className="mb-6">
                              ...
                              <motion.button
                                ...
                                className="w-full bg-gradient-to-r from-[#0e7490] via-[#0891b2] to-[#155e75] text-white font-semibold py-3 sm:py-4 rounded-2xl hover:shadow-2xl hover:shadow-[#0e7490]/30 transition-all duration-500 transform hover:-translate-y-1 flex items-center justify-center gap-2 mt-4 text-sm sm:text-base"
                            ...
                            <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-md border border-[#493657]/10">
                                <div className="flex flex-col">
                                    <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
                                        <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-[#06b6d4] to-[#0891b2] rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0">
                                    ...
                                    <div className="space-y-3 sm:space-y-4">
                                        {Array.isArray(product.advantages) && product.advantages.length > 0 ? (
                                            product.advantages.map((adv, idx) => (
                                                <div key={idx} className="flex items-start gap-3 p-3 sm:p-4 bg-[#f8fafc] rounded-lg border-l-4 border-[#06b6d4]">
                                                    <div className="w-5 h-5 sm:w-6 sm:h-6 bg-[#06b6d4] rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                ...
                <div className="mt-8 sm:mt-12 lg:mt-16">
                    <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-[#493657]/10 overflow-hidden">
                        <div className="bg-gradient-to-r from-[#0e7490] via-[#0891b2] to-[#155e75] text-white p-4 sm:p-6 relative overflow-hidden">
                        ...
                        <div className="grid grid-cols-1 gap-px bg-[#e2e8f0]">
                            {[
                                { label: "Category", value: "Industrial Coating", icon: FiShield },
                                ...
                                <div key={idx} className="bg-white p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-0 hover:bg-[#f8fafc] transition-colors">
                                    <div className="flex items-center gap-3">
                                        <spec.icon className="w-4 h-4 sm:w-5 sm:h-5 text-[#0891b2]" />
                ...
                <div className="mt-8 sm:mt-12 lg:mt-16">
                    <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-[#493657]/10 overflow-hidden">
                        <div className="bg-gradient-to-r from-[#0e7490] via-[#0891b2] to-[#155e75] text-white p-4 sm:p-6 text-center relative overflow-hidden">
                            <div className="absolute inset-0 bg-gradient-to-b from-transparent via-white/5 to-black/20"></div>
                        ...
                        <div className="grid grid-cols-1 gap-px bg-[#e2e8f0]">
                            {[
                                ...
                                { label: "Pot Life", value: product.technicalSpecs?.pot_life, icon: FiClock },
                    ...
                    <div className="bg-gradient-to-br from-[#f8fafc] to-[#f1f5f9] rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 shadow-lg border border-[#493657]/10">
                        ...
                        <div className="grid grid-cols-1 gap-6 sm:gap-8 md:grid-cols-2">
                            ...
                            <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg border border-[#493657]/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group">
                                ...
                                <a
                                    href="/Assets/docs/html-templates/calyco-epoxy-paint-tds.html"
                                    download
                            ...
                            <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg border border-[#493657]/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group">
                                ...
                                <a
                                    href="/Assets/docs/html-templates/calyco-epoxy-paint-sds.html"
                                    download
                ...
                {/* ENHANCED Features Section - Adapted for Epoxy Paint */}
                <motion.div
                    className="mt-8 sm:mt-12 lg:mt-16"
                    variants={itemVariants}
                >
                    <div className="relative rounded-2xl sm:rounded-3xl bg-gradient-to-br from-[#06b6d4]/5 via-white to-[#4f46e5]/5 p-6 sm:p-8 md:p-12 shadow-2xl overflow-hidden border border-[#493657]/10">
                        {/* Premium Background Effects - Cyan/Blue Tones */}
                        <div className="pointer-events-none absolute -top-24 -right-12 w-96 h-96 bg-gradient-to-br from-[#06b6d4]/20 to-[#493657]/10 blur-3xl rounded-full" />
                        <div className="pointer-events-none absolute -bottom-32 -left-10 w-80 h-80 bg-gradient-to-tr from-[#4f46e5]/15 to-[#06b6d4]/10 blur-3xl rounded-full" />
                        <div className="pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-gradient-to-r from-[#0891b2]/10 to-[#1e3a8a]/10 blur-2xl rounded-full" />

                        <div className="relative z-10">
                            <h2 className="text-3xl sm:text-4xl font-bold text-[#493657] mb-3 sm:mb-4 text-center">Engineered Toughness</h2>
                            <p className="text-center text-[#493657]/70 mb-8 sm:mb-12 text-base sm:text-lg">Professional grade performance for industrial needs</p>

                            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-4 sm:gap-8">
                                {/* Feature 1: Chemical Resistance */}
                                <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#06b6d4] transition-all duration-500 hover:-translate-y-2">
                                    <div className="w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-[#06b6d4] via-[#22d3ee] to-[#67e8f9] rounded-xl sm:rounded-2xl flex items-center justify-center mb-4 sm:mb-6 group-hover:scale-110 transition-transform duration-300 shadow-lg">
                                        <FiShield className="w-7 h-7 sm:w-8 sm:h-8 text-white drop-shadow-sm" />
                                    </div>
                                    <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">Chemical Resist</h3>
                                    <p className="text-sm text-[#493657]/70 leading-relaxed">Impervious to oils, solvents, and industrial chemicals that would damage standard paints.</p>
                                    <div className="mt-3 sm:mt-4 h-1 w-10 sm:w-12 bg-gradient-to-r from-[#06b6d4] to-[#06b6d4]/40 rounded-full group-hover:w-14 sm:group-hover:w-16 transition-all duration-300"></div>
                                </div>

                                {/* Feature 2: Abrasion Resistance */}
                                <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#06b6d4] transition-all duration-500 hover:-translate-y-2">
                                    <div className="w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-[#4f46e5] via-[#6366f1] to-[#818cf8] rounded-xl sm:rounded-2xl flex items-center justify-center mb-4 sm:mb-6 group-hover:scale-110 transition-transform duration-300 shadow-lg">
                                        <FiHexagon className="w-7 h-7 sm:w-8 sm:h-8 text-white drop-shadow-sm" />
                                    </div>
                                    <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">High Strength</h3>
                                    <p className="text-sm text-[#493657]/70 leading-relaxed">Designed for high-traffic areas, resisting wear from forklifts, cars, and heavy machinery.</p>
                                    <div className="mt-3 sm:mt-4 h-1 w-10 sm:w-12 bg-gradient-to-r from-[#4f46e5] to-[#4f46e5]/40 rounded-full group-hover:w-14 sm:group-hover:w-16 transition-all duration-300"></div>
                                </div>

                                {/* Feature 3: 2-Component */}
                                <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#06b6d4] transition-all duration-500 hover:-translate-y-2">
                                    <div className="w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-[#0891b2] via-[#0e7490] to-[#155e75] rounded-xl sm:rounded-2xl flex items-center justify-center mb-4 sm:mb-6 group-hover:scale-110 transition-transform duration-300 shadow-lg">
                                        <FiLayers className="w-7 h-7 sm:w-8 sm:h-8 text-white drop-shadow-sm" />
                                    </div>
                                    <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">2-Part System</h3>
                                    <p className="text-sm text-[#493657]/70 leading-relaxed">Resin and hardener combine to create a cross-linked polymer bond far stronger than 1-part paints.</p>
                                    <div className="mt-3 sm:mt-4 h-1 w-10 sm:w-12 bg-gradient-to-r from-[#0891b2] to-[#0891b2]/40 rounded-full group-hover:w-14 sm:group-hover:w-16 transition-all duration-300"></div>
                                </div>

                                {/* Feature 4: High Gloss */}
                                <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#06b6d4] transition-all duration-500 hover:-translate-y-2">
                                    <div className="w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-[#059669] via-[#10b981] to-[#34d399] rounded-xl sm:rounded-2xl flex items-center justify-center mb-4 sm:mb-6 group-hover:scale-110 transition-transform duration-300 shadow-lg">
                                        <FiZap className="w-7 h-7 sm:w-8 sm:h-8 text-white drop-shadow-sm" />
                                    </div>
                                    <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">High Gloss</h3>
                                    <p className="text-sm text-[#493657]/70 leading-relaxed">Provides a bright, professional finish that reflects light and is incredibly easy to clean.</p>
                    ...
                    <div className="relative max-w-6xl mx-auto rounded-2xl sm:rounded-3xl bg-white/90 backdrop-blur-md border border-white/70 shadow-2xl px-4 py-6 sm:px-6 sm:py-10 md:px-10">
                        <div className="flex flex-col gap-4 sm:gap-6 mb-6 sm:mb-10 md:flex-row md:items-center md:justify-between">
                            <div>
                                ...
                                <p className="text-sm md:text-base text-[#493657]/70 mt-2">Finish your industrial project with our premium range.</p>
                            </div>
