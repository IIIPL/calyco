...
const solventPrimerDetail = {
    ...
    advantages: [
        "Excellent adhesion to plaster, wood, and metal",
        "Seals porous surfaces and boosts topcoat coverage",
        "Superior whiteness and opacity for hiding power",
        "Quick drying with low VOC for solvent class",
    ],
    application: ["Interior masonry", "Wood surfaces", "Metal surfaces", "Plaster", "Gypsum boards"],
    recommended_uses: ["Interior masonry", "Wood surfaces", "Metal surfaces", "Plaster", "Gypsum boards"],
    ...
    technicalSpecs: {
        ...
        recoatTime: "6 hours",
        application_instructions: "Stir well. Thin with mineral turpentine if required (max 5-10%). Apply 1-2 uniform coats by brush, roller, or spray.",
        shelf_life: "3 Years",
        storage_temp: "Cool, dry place away from heat and flame",
        cleanup: "Mineral turpentine",
    },
    safety_warnings: {
        hazard_statements: ["Flammable liquid and vapour.", "May cause skin irritation on prolonged contact."],
...
const CalycoSolventPrimerInterior = () => {
    ...
    useEffect(() => {
        setProduct(solventPrimerDetail);
        setSelectedSheen(solventPrimerDetail.defaultFinish || "Matte");

        const finishPricing = (solventPrimerDetail.priceByFinish || solventPrimerDetail.price_by_finish || {})["Matte"];
        if (finishPricing && typeof finishPricing === "object") {
            const sizeKeys = Object.keys(finishPricing);
            if (sizeKeys.length > 0) {
                setSelectedSize(sizeKeys[0]);
            } else if (solventPrimerDetail.packaging && solventPrimerDetail.packaging.length > 0) {
                setSelectedSize(solventPrimerDetail.packaging[0]);
            }
        } else if (solventPrimerDetail.packaging && solventPrimerDetail.packaging.length > 0) {
            setSelectedSize(solventPrimerDetail.packaging[0]);
        }

        if (Array.isArray(solventPrimerDetail.images) && solventPrimerDetail.images.length > 0) {
            setSelectedImage(solventPrimerDetail.images[0]);
            setSelectedImageIndex(0);
        } else {
            setSelectedImage(solventPrimerDetail.image);
            setSelectedImageIndex(0);
        }
        document.title = solventPrimerDetail.name;
        setLoading(false);
    }, []);
                            ...
                            <div className="grid grid-cols-1 gap-6 sm:gap-8 md:grid-cols-2">
                                ...
                                <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg border border-[#493657]/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2 mt-3">
                                        <a href="/Assets/docs/html-templates/calyco-solvent-primer-interior-tds.html" download className="flex-1 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiDownload className="w-4 h-4 sm:w-5 sm:h-5" />
                                            Download TDS
                                        </a>
                                        <a href="/Assets/docs/html-templates/calyco-solvent-primer-interior-tds.html" target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                ...
                                <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg border border-[#493657]/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2 mt-3">
                                        <a href="/Assets/docs/html-templates/calyco-solvent-primer-interior-sds.html" download className="flex-1 bg-gradient-to-r from-[#f59e0b] to-[#d97706] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiDownload className="w-4 h-4 sm:w-5 sm:h-5" />
                                            Download SDS
                                        </a>
                                        <a href="/Assets/docs/html-templates/calyco-solvent-primer-interior-sds.html" target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-r from-[#f59e0b] to-[#d97706] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiExternalLink className="w-4 h-4 sm:w-5 sm:h-5" />
                                            View SDS
                                        </a>
                                    </div>
...
export default CalycoSolventPrimerInterior;
