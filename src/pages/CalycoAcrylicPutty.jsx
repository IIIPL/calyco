...
// We don't import colors for Putty as it is standard white
// import { calycoColors as colorData } from "../data/calycoColors.js"; 

// --- DATA DEFINITION FOR ACRYLIC WALL PUTTY ---
const acrylicPuttyDetail = {
  ...
  features: [
    "Ready-to-use paste format",
    "Superior whiteness & smoothness",
    "Excellent filling properties",
    "High adhesive strength",
  ],
  advantages: [
    "Ensures smooth, butter-like finish",
    "Enhances true look of topcoat paints",
    "Does not require curing",
    "Reduces paint consumption",
    "Breathable and alkali resistant",
  ],
  ...
  safety_warnings: {
    hazard_statements: ["Avoid breathing dust when sanding.", "May cause eye irritation."],
    precautionary_statements: ["Wear dust mask while sanding.", "Use in well-ventilated areas.", "Keep container tightly closed."],
    first_aid: {
        inhalation: "Move to fresh air if dust is inhaled.",
        skin_contact: "Wash with soap and water.",
        eye_contact: "Rinse eyes cautiously with water.",
...
const CalycoAcrylicPutty = () => {
    ...
    const [selectedSheen, setSelectedSheen] = useState("Smooth");
    ...
    }, [product?.id, selectedSheen, selectedSize, quantity]);
    ...
    const normalizedSelectedSheen = selectedSheen || product?.defaultFinish || "Smooth";
    ...
    // Calculate MRP for Acrylic Putty
    const calculateMRP = (sizeLabel) => {
        const mrpData = PUTTY_MRP[normalizedSelectedSheen];
    ...
    useEffect(() => {
        setProduct(acrylicPuttyDetail);
        setSelectedSheen(acrylicPuttyDetail.defaultFinish || "Smooth");

        const finishPricing = (acrylicPuttyDetail.priceByFinish || acrylicPuttyDetail.price_by_finish || {})["Smooth"];
        if (finishPricing && typeof finishPricing === "object") {
            const sizeKeys = Object.keys(finishPricing);
            if (sizeKeys.length > 0) {
                setSelectedSize(sizeKeys[0]);
            } else if (acrylicPuttyDetail.packaging && acrylicPuttyDetail.packaging.length > 0) {
                setSelectedSize(acrylicPuttyDetail.packaging[0]);
            }
        } else if (acrylicPuttyDetail.packaging && acrylicPuttyDetail.packaging.length > 0) {
            setSelectedSize(acrylicPuttyDetail.packaging[0]);
        }

        if (Array.isArray(acrylicPuttyDetail.images) && acrylicPuttyDetail.images.length > 0) {
            setSelectedImage(acrylicPuttyDetail.images[0]);
            setSelectedImageIndex(0);
        } else {
            setSelectedImage(acrylicPuttyDetail.image);
            setSelectedImageIndex(0);
        }
        document.title = acrylicPuttyDetail.name;
        setLoading(false);
    }, []);
    ...
    const handleAddToCart = () => {
        ...
        setCartPopup({
            ...
            item: {
                name: product.name,
                hex: "#FFFFFF", // Putty is always white
                colorName: "Standard White",
                colorFamily: "Base",
                selectedSheen,
                selectedSize,
                selectedColorType,
            ...
            >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-16 items-start">
                    ...
                    >
                        ...
                        <div>
                            ...
                            {displayMRPValue ? (
                              <div className="flex flex-wrap items-baseline gap-2 sm:gap-3">
                                ...
                                <span className="text-xs sm:text-sm text-[#493657]/60">per {selectedSize || displaySizes[0] || '1 kg'}</span>
                              </div>
                            ) : (
                              <div className="flex flex-wrap items-baseline gap-2 sm:gap-3">
                                <span className="text-2xl sm:text-3xl md:text-4xl font-bold text-[#493657]">{formatINR(displayPriceValue)}</span>
                        ...
                        {/* 3 Feature Cards - Putty Specific */}
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 my-3">
                          <div className="bg-[#00BCD4] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                            ...
                            <h4 className="font-bold text-white text-sm sm:text-base mb-1 drop-shadow-sm">Ultra Smooth</h4>
                            <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">Butter-like finish</p>
                          </div>
                          <div className="bg-[#FF9500] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                            ...
                            <h4 className="font-bold text-white text-sm sm:text-base mb-1 drop-shadow-sm">Strong Grip</h4>
                            <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">Superior adhesion</p>
                          </div>
                          <div className="bg-[#34C759] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                            <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-md">
                              <FiSun className="w-6 h-6 sm:w-7 sm:h-7 text-[#34C759]" />
                            </div>
                            <h4 className="font-bold text-white text-sm sm:text-base mb-1 drop-shadow-sm">Whiteness</h4>
                            <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">Bright base</p>
                          </div>
                        </div>
                        ...
                        <div className="space-y-4 sm:space-y-6">
                            ...
                            {/* Color Selection - Hidden for Putty as it is white */}
                                ...
                                >
                                  <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1">
                                      <h4 className="font-semibold text-[#493657] mb-1 text-sm sm:text-base">Standard White Putty</h4>
                                      <p className="text-xs text-[#493657]/70 leading-relaxed">
                                        Factory standard bright white acrylic putty. Ready to use.
                                      </p>
                ...
                <div className="mt-8 sm:mt-12 lg:mt-16">
                    <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-[#493657]/10 overflow-hidden">
                        ...
                        <div className="grid grid-cols-1 gap-px bg-[#e2e8f0]">
                            {[
                                { label: "Category", value: "Wall Putty", icon: FiTag },
                        ...
                        <div className="grid grid-cols-1 gap-6 sm:gap-8 md:grid-cols-2">
                            ...
                            <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg border border-[#493657]/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group">
                                <div className="flex items-center gap-3 sm:gap-4 mb-4">
                                    ...
                                    <a href="/Assets/docs/html-templates/calyco-acrylic-wall-putty-tds.html" download className="flex-1 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                        <FiDownload className="w-4 h-4 sm:w-5 sm:h-5" />
                                        Download TDS
                                    </a>
                                    <a href="/Assets/docs/html-templates/calyco-acrylic-wall-putty-tds.html" target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                ...
                {/* ENHANCED Features Section - Adapted for Putty */}
                <motion.div
                    className="mt-8 sm:mt-12 lg:mt-16"
                    variants={itemVariants}
                >
                    <div className="relative rounded-2xl sm:rounded-3xl bg-gradient-to-br from-[#493657]/5 via-white to-[#F0C85A]/5 p-6 sm:p-8 md:p-12 shadow-2xl overflow-hidden border border-[#493657]/10">
                        ...
                        <div className="relative z-10">
                            ...
                            <p className="text-center text-[#493657]/70 mb-8 sm:mb-12 text-base sm:text-lg">Experience the perfect smoothness for your walls</p>

                            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-4 sm:gap-8">
                                {/* Feature 1: Ultra-Smooth Finish */}
                                <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                    ...
                                    <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">Ultra-Smooth Finish</h3>
                                    <p className="text-sm text-[#493657]/70 leading-relaxed">Provides a butter-like finish that requires minimal sanding.</p>
                                ...
                                <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                    ...
                                    <p className="text-sm text-[#493657]/70 leading-relaxed">Strong bonding with base plaster ensures long-lasting durability.</p>
                                ...
                                <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                    ...
                                    <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">Super Whiteness</h3>
                                    <p className="text-sm text-[#493657]/70 leading-relaxed">Provides a bright white base that enhances the true tone of topcoat paint.</p>
                                ...
                                <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                    ...
                                    <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">Breathable</h3>
                                    <p className="text-sm text-[#493657]/70 leading-relaxed">Allows moisture to escape from the wall, preventing dampness and peeling.</p>
                    ...
                    <div className="relative max-w-6xl mx-auto rounded-2xl sm:rounded-3xl bg-white/90 backdrop-blur-md border border-white/70 shadow-2xl px-4 py-6 sm:px-6 sm:py-10 md:px-10">
                        <div className="flex flex-col gap-4 sm:gap-6 mb-6 sm:mb-10 md:flex-row md:items-center md:justify-between">
                            <div>
                                ...
                                <p className="text-sm md:text-base text-[#493657]/70 mt-2">Complete your wall preparation with primer and topcoat.</p>
                            </div>
...
export default CalycoAcrylicPutty;
