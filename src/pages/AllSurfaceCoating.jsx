...
// --- DATA DEFINITION FOR ALL-SURFACE COATING (Embedded for self-containment) ---
const allSurfaceDetail = {
    id: "calyco-all-surface-coating",
    ...
    tagline: "One paint for every surface.",
    ...
    advantages: [
        "Adheres to wood, metal, plastic, masonry, and walls",
        "Weatherproof and UV resistant",
        "Saves time with built-in primer",
        "Low odour, quick drying for faster completion",
    ],
    application: ["Wood surfaces", "Metal surfaces", "Plastic", "Masonry", "Concrete", "Interior & exterior walls"],
    recommended_uses: ["Wood surfaces", "Metal surfaces", "Plastic", "Masonry", "Concrete", "Interior & exterior walls"],
    ...
    technicalSpecs: {
        ...
        voc_content: "≤ 100 g/L",
        dryingTime: "1 hour (touch dry)",
        recoatTime: "4 hours",
        application_instructions: "Stir well. Dilute up to 10-15% with clean water if required. Apply 2 coats by brush/roller/spray with 4 hours between coats. Built-in primer covers most surfaces; adjust prep per substrate.",
        shelf_life: "3 Years",
        storage_temp: "Cool, dry place; avoid freezing/extreme heat",
        cleanup: "Clean water",
    },
    safety_warnings: {
        hazard_statements: ["May cause skin/eye irritation."],
        precautionary_statements: ["Keep out of reach of children.", "Use in well-ventilated areas.", "Avoid contact with eyes and skin.", "Store in cool, dry place; protect from freezing/heat."],
        first_aid: {
...
const AllSurfaceCoating = () => {
    ...
    const calculateMRP = (sizeLabel) => {
        const mrpData = ALL_SURFACE_MRP[normalizedSelectedSheen];
    ...
    useEffect(() => {
        setProduct(allSurfaceDetail);
        setSelectedSheen(allSurfaceDetail.defaultFinish || "Semi-Gloss/Satin");

        const finishPricing = (allSurfaceDetail.priceByFinish || allSurfaceDetail.price_by_finish || {})["Semi-Gloss/Satin"];
        if (finishPricing && typeof finishPricing === "object") {
            const sizeKeys = Object.keys(finishPricing);
            if (sizeKeys.length > 0) {
                setSelectedSize(sizeKeys[0]);
            } else if (allSurfaceDetail.packaging && allSurfaceDetail.packaging.length > 0) {
                setSelectedSize(allSurfaceDetail.packaging[0]);
            }
        } else if (allSurfaceDetail.packaging && allSurfaceDetail.packaging.length > 0) {
            setSelectedSize(allSurfaceDetail.packaging[0]);
        }

        if (Array.isArray(allSurfaceDetail.images) && allSurfaceDetail.images.length > 0) {
            setSelectedImage(allSurfaceDetail.images[0]);
            setSelectedImageIndex(0);
        } else {
            setSelectedImage(allSurfaceDetail.image);
            setSelectedImageIndex(0);
        }
        document.title = allSurfaceDetail.name;
        setLoading(false);
    }, []);
                ...
                >
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-16 items-start">
                                        ...
                                        {product.images && product.images.length > 1 && (
                                            <>
                                                <button
                                                    ...
                                                    className={`absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-white shadow-lg flex items-center justify-center transition-all duration-200 hover:bg-[#F0C85A]/10 hover:shadow-xl ${selectedImageIndex === 0
                                                ...
                                                <button
                                                    ...
                                                    className={`absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-white shadow-lg flex items-center justify-center transition-all duration-200 hover:bg-[#F0C85A]/10 hover:shadow-xl ${selectedImageIndex === product.images.length - 1
                                                        ? 'opacity-30 cursor-not-allowed'
                                                        : 'opacity-100'
                                                        }`}
                                        ...
                                        <div className="flex justify-center gap-1.5 sm:gap-2 mt-3 sm:mt-4">
                                            {product.images.map((_, idx) => (
                                                <button
                                                    ...
                                                    className={`w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full transition-all duration-200 ${idx === selectedImageIndex
                                        ...
                                        >
                                            {product.images.map((img, idx) => (
                                                <button
                                                    ...
                                                    className={`rounded-md sm:rounded-lg p-1.5 sm:p-2 transition-all duration-200 focus:outline-none flex-shrink-0 w-20 sm:w-24 ${selectedImageIndex === idx
                                                        ? 'bg-[#F0C85A]/10'
                        ...
                        >
                            ...
                            {/* 3 Feature Cards - All Surface Specific */}
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 my-3">
                                <div className="bg-[#FF9500] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                                    ...
                                    <h4 className="font-bold text-white text-sm sm:text-base mb-1 drop-shadow-sm">Multi-Surface</h4>
                                    <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">Wood, Metal, Wall</p>
                                </div>
                                <div className="bg-[#2563eb] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                                    <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-md">
                                        <FiCheckCircle className="w-6 h-6 sm:w-7 sm:h-7 text-[#2563eb]" />
                                    </div>
                                    <h4 className="font-bold text-white text-sm sm:text-base mb-1 drop-shadow-sm">Self-Priming</h4>
                                    <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">Built-in primer</p>
                                </div>
                                <div className="bg-[#34C759] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                                    <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-md">
                                        <FiWind className="w-6 h-6 sm:w-7 sm:h-7 text-[#34C759]" />
                                    </div>
                                    <h4 className="font-bold text-white text-sm sm:text-base mb-1 drop-shadow-sm">Weather Proof</h4>
                                    <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">UV resistant</p>
                                </div>
                            </div>
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <div className="grid grid-cols-1 gap-3">
                                            <button
                                                ...
                                                className={`relative p-3 sm:p-4 rounded-xl border-2 transition-all duration-300 text-left ${selectedColorType === "ready-mixed"
                                                    ? "border-[#F0C85A] bg-[#F0C85A]/5 shadow-lg"
                                                    : "border-[#493657]/20 hover:border-[#493657]/40 hover:shadow-md"
                                                    }`}
                                            >
                                                <div className="flex items-start gap-3">
                                                    <div className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full border-2 flex items-center justify-center mt-0.5 ${selectedColorType === "ready-mixed"
                                            ...
                                            <button
                                                ...
                                                className={`relative p-3 sm:p-4 rounded-xl border-2 transition-all duration-300 text-left ${selectedColorType === "tint-on-demand"
                                                    ? "border-[#F0C85A] bg-[#F0C85A]/5 shadow-lg"
                                                    : "border-[#493657]/20 hover:border-[#493657]/40 hover:shadow-md"
                                                    }`}
                                            >
                                                <div className="flex items-start gap-3">
                                                    <div className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full border-2 flex items-center justify-center mt-0.5 ${selectedColorType === "tint-on-demand"
                                                        ? "border-[#F0C85A] bg-[#F0C85A]"
                                                        : "border-[#493657]/30"
                                                        }`}>
                            ...
                            <div className="grid grid-cols-1 gap-6 sm:gap-8 md:grid-cols-2">
                                ...
                                <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg border border-[#493657]/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2 mt-3">
                                        <a href="/Assets/docs/html-templates/calyco-all-surface-coating-tds.html" download className="flex-1 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiDownload className="w-4 h-4 sm:w-5 sm:h-5" />
                                            Download TDS
                                        </a>
                                        <a href="/Assets/docs/html-templates/calyco-all-surface-coating-tds.html" target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                ...
                                <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg border border-[#493657]/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2 mt-3">
                                        <a href="/Assets/docs/html-templates/calyco-all-surface-coating-sds.html" download className="flex-1 bg-gradient-to-r from-[#f59e0b] to-[#d97706] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiDownload className="w-4 h-4 sm:w-5 sm:h-5" />
                                            Download SDS
                                        </a>
                                        <a href="/Assets/docs/html-templates/calyco-all-surface-coating-sds.html" target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-r from-[#f59e0b] to-[#d97706] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiExternalLink className="w-4 h-4 sm:w-5 sm:h-5" />
                                            View SDS
                                        </a>
                                    </div>
                        ...
                        <div className="relative rounded-2xl sm:rounded-3xl bg-gradient-to-br from-[#493657]/5 via-white to-[#F0C85A]/5 p-6 sm:p-8 md:p-12 shadow-2xl overflow-hidden border border-[#493657]/10">
                            ...
                            <div className="relative z-10">
                                ...
                                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-4 sm:gap-8">
                                    {/* Feature 1: Multi-Surface Adhesion (Gold) */}
                                    <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                        ...
                                        <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">Multi-Surface</h3>
                                        <p className="text-sm text-[#493657]/70 leading-relaxed">Direct application on wood, metal, plastic, masonry, and walls.</p>
                                    ...
                                    {/* Feature 2: Built-in Primer (Purple) */}
                                    <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                        ...
                                        <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">Built-in Primer</h3>
                                        <p className="text-sm text-[#493657]/70 leading-relaxed">Self-priming formula saves time and effort on preparation.</p>
                                    ...
                                    {/* Feature 3: Weather Resistant (Green) */}
                                    <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                        ...
                                        <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">Weather Proof</h3>
                                        <p className="text-sm text-[#493657]/70 leading-relaxed">Resistant to UV radiation and rain, suitable for exterior use.</p>
                                    ...
                                    <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                        ...
                                        <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">Low Odour</h3>
                                        <p className="text-sm text-[#493657]/70 leading-relaxed">Water-based hybrid technology for a pleasant painting experience.</p>
