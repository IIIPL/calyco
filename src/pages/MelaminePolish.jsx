...
import { FiTag, FiList, FiCheckCircle, FiDroplet, FiClipboard, FiLayers, FiBox, FiPackage, FiShield, FiArchive, FiAlertCircle, FiInfo, FiCalendar, FiHeart, FiChevronLeft, FiChevronRight, FiWind, FiDownload, FiClock, FiThermometer, FiStar, FiAlertTriangle, FiExternalLink } from 'react-icons/fi';
...
const melaminePolishDetail = {
    ...
    features: [
        ...
        "Fast Drying (Touch Dry 30min)",
        "Stain Resistant Protection"
    ],
    advantages: [
        "Superior scratch and abrasion resistance",
        "Excellent heat and stain resistance",
        "Brilliant high-gloss mirror finish",
        "Fast drying for quick application",
        "Self-leveling for smooth surface",
        "Easy to apply and maintain"
    ],
    application: ["Wooden Furniture", "Kitchen Cabinets", "Modular Furniture", "Interior Woodwork"],
    recommended_uses: ["Dining Tables", "Doors & Frames", "Paneling", "Wardrobes"],
    substrate: ["Wood", "Veneer", "Plywood", "MDF"],
    coats_required: "2-3 coats",
    coverage: "100-120 sq.ft./L",
    technicalSpecs: {
        product_code: "CAL-MEL-008",
        base_type: "Melamine Resin",
        voc_content: "≤ 300 g/L",
        dryingTime: "30-45 minutes (Touch Dry)",
        recoatTime: "4-6 hours",
        full_cure: "24-48 hours",
        application_instructions: "Stir gently. Thin max 10-15% with NC thinner. Spray application recommended for best results.",
        shelf_life: "3 Years (Warranty)",
        storage_temp: "Cool, dry place",
        cleanup: "NC Thinner",
        preparation_instructions: "Sand 120-220 grit. Remove wax/oil. Apply sealer if porous. Ensure dust-free before coating."
    },
    safety_warnings: {
        hazard_statements: [
        ...
        first_aid: {
            ...
            skin_contact: "Remove contaminated clothing. Rinse skin with water/shower.",
            eye_contact: "Rinse cautiously with water for several minutes. Remove contact lenses if easy to do.",
            ingestion: "Immediately call a POISON CENTER or doctor. Do NOT induce vomiting."
        }
    }
};
...
const MelaminePolish = () => {
    // Logic for colors - Melamine is usually Clear
    ...
    const calculateMRP = (sizeLabel) => {
        const mrpData = MELAMINE_MRP[normalizedSelectedSheen];
    ...
    useEffect(() => {
        setProduct(melaminePolishDetail);
        setSelectedSheen(melaminePolishDetail.defaultFinish || "High Gloss");

        const finishPricing = (melaminePolishDetail.priceByFinish || melaminePolishDetail.price_by_finish || {})["High Gloss"];
        if (finishPricing && typeof finishPricing === "object") {
            const sizeKeys = Object.keys(finishPricing);
            if (sizeKeys.length > 0) {
                // Ensure 500ml is selected first if available
                if (sizeKeys.includes("500ml")) {
                    setSelectedSize("500ml");
                } else {
                    setSelectedSize(sizeKeys[0]);
                }
            } else if (melaminePolishDetail.packaging && melaminePolishDetail.packaging.length > 0) {
                setSelectedSize(melaminePolishDetail.packaging[0]);
            }
        } else if (melaminePolishDetail.packaging && melaminePolishDetail.packaging.length > 0) {
            setSelectedSize(melaminePolishDetail.packaging[0]);
        }

        if (Array.isArray(melaminePolishDetail.images) && melaminePolishDetail.images.length > 0) {
            setSelectedImage(melaminePolishDetail.images[0]);
            setSelectedImageIndex(0);
        } else {
            setSelectedImage(melaminePolishDetail.image);
            setSelectedImageIndex(0);
        }
        document.title = melaminePolishDetail.name;
        setLoading(false);
    }, []);
    ...
    const handleAddToCart = () => {
        ...
        setCartPopup({
            ...
            item: {
                name: product.name,
                hex: "#EAD2AC", // Wood/Varnish color approximation
                colorName: "Clear Gloss",
                colorFamily: "Wood Finish",
                selectedSheen,
                selectedSize,
                selectedColorType,
                quantity,
                ...
                >
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-16 items-start">
                                        ...
                                        <img
                                            ...
                                            onError={(e) => {
                                                ...
                                                if (!e.target.src.includes("placeholder")) {
                                                    e.target.src = "https://placehold.co/600x600?text=Melamine+Polish";
                                                }
                                            }}
                        ...
                        >
                            ...
                            <div>
                                ...
                                {displayMRPValue ? (
                                    <div className="flex flex-wrap items-baseline gap-2 sm:gap-3">
                                        ...
                                        <span className="text-xs sm:text-sm text-[#493657]/60">per {selectedSize || displaySizes[0] || '10L'}</span>
                                    </div>
                                ) : (
                                    <div className="flex flex-wrap items-baseline gap-2 sm:gap-3">
                                        <span className="text-2xl sm:text-3xl md:text-4xl font-bold text-[#493657]">{formatINR(displayPriceValue)}</span>
                            ...
                            {/* 3 Feature Cards - Melamine Specific */}
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 my-3">
                                <div className="bg-[#6366F1] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                                    <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-md">
                                        <FiStar className="w-6 h-6 sm:w-7 sm:h-7 text-[#6366F1]" />
                                    </div>
                                    <h4 className="font-bold text-white text-sm sm:text-base mb-1 drop-shadow-sm">High Gloss</h4>
                                    <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">Mirror finish</p>
                                </div>
                                <div className="bg-[#EC4899] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                                    <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-md">
                                        <FiShield className="w-6 h-6 sm:w-7 sm:h-7 text-[#EC4899]" />
                                    </div>
                                    <h4 className="font-bold text-white text-sm sm:text-base mb-1 drop-shadow-sm">Scratch Resist</h4>
                                    <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">Hard coating</p>
                                </div>
                                <div className="bg-[#F59E0B] rounded-xl p-4 sm:p-5 text-center shadow-lg border border-[#493657]/10 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                                    ...
                                    <p className="text-xs sm:text-sm text-white/90 font-medium drop-shadow-sm">Protects wood</p>
                                </div>
                            </div>
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {product.finish_type_sheen.map((sheen) => (
                                                <button
                                                    ...
                                                    className={`px-3 sm:px-4 py-2 rounded-lg border transition-all text-sm sm:text-base ${selectedSheen === sheen
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {displaySizes.map((size) => (
                                                <button
                                                    ...
                                                    className={`px-3 sm:px-4 py-2 rounded-lg border transition-all text-sm sm:text-base ${selectedSize === size
                                                        ? "border-[#F0C85A] bg-[#F0C85A]/10 text-[#493657]"
                                                        : "border-[#493657]/20 text-[#493657]/70 hover:border-[#493657]/40"
                                                        }`}
                                                >
                    ...
                    <div className="mt-8 sm:mt-12 lg:mt-16">
                        <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-[#493657]/10 overflow-hidden">
                            ...
                            <div className="grid grid-cols-1 gap-px bg-[#e2e8f0]">
                                {[
                                    { label: "Category", value: "Wood Polish", icon: FiTag },
                    ...
                    {SHOW_SAFETY_SECTION && product.safety_warnings && (
                        <div className="mt-8 sm:mt-12 lg:mt-16">
                            <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-[#493657]/10 overflow-hidden">
                                <div className="bg-gradient-to-r from-[#ef4444] to-[#b91c1c] text-white p-4 sm:p-6 flex items-center gap-3 sm:gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-red-100 mt-1 text-sm sm:text-base">Please read all safety information before use</p>
                                    </div>
                                </div>
                                        ...
                                        {product.safety_warnings.hazard_statements && (
                                            <div>
                                                <h3 className="font-bold text-[#493657] text-lg sm:text-xl mb-4 flex items-center gap-3">
                                                    <FiAlertCircle className="w-5 h-5 sm:w-6 sm:h-6 text-[#ef4444]" />
                                                    Hazard Statements
                                                </h3>
                                                <div className="space-y-2">
                                                    {product.safety_warnings.hazard_statements.map((statement, idx) => (
                                                        <div key={idx} className="flex items-start gap-2 sm:gap-3 p-2 sm:p-3 bg-[#fee2e2] border-l-4 border-[#ef4444] rounded-r-lg">
                                                            <div className="w-4 h-4 sm:w-5 sm:h-5 bg-[#ef4444] rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                            ...
                            <div className="grid grid-cols-1 gap-6 sm:gap-8 md:grid-cols-2">
                                ...
                                <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg border border-[#493657]/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2 mt-3">
                                        <a href="/Assets/docs/html-templates/calyco-melamine-polish-tds.html" download className="flex-1 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiDownload className="w-4 h-4 sm:w-5 sm:h-5" />
                                            Download TDS
                                        </a>
                                        <a href="/Assets/docs/html-templates/calyco-melamine-polish-tds.html" target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                ...
                                <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg border border-[#493657]/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2 mt-3">
                                        <a href="/Assets/docs/html-templates/calyco-melamine-polish-sds.html" download className="flex-1 bg-gradient-to-r from-[#f59e0b] to-[#d97706] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiDownload className="w-4 h-4 sm:w-5 sm:h-5" />
                                            Download SDS
                                        </a>
                                        <a href="/Assets/docs/html-templates/calyco-melamine-polish-sds.html" target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-r from-[#f59e0b] to-[#d97706] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 font-semibold text-sm sm:text-base">
                                            <FiExternalLink className="w-4 h-4 sm:w-5 sm:h-5" />
                                            View SDS
                                        </a>
                                    </div>
                    ...
                    {/* ENHANCED Features Section - Adapted for Melamine Polish */}
                    <motion.div
                        className="mt-8 sm:mt-12 lg:mt-16"
                        variants={itemVariants}
                    >
                        <div className="relative rounded-2xl sm:rounded-3xl bg-gradient-to-br from-[#493657]/5 via-white to-[#F0C85A]/5 p-6 sm:p-8 md:p-12 shadow-2xl overflow-hidden border border-[#493657]/10">
                            ...
                            <div className="relative z-10">
                                <h2 className="text-3xl sm:text-4xl font-bold text-[#493657] mb-3 sm:mb-4 text-center">Premium Wood Finish</h2>
                                <p className="text-center text-[#493657]/70 mb-8 sm:mb-12 text-base sm:text-lg">Lasting brilliance for your cherished furniture</p>

                                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-4 sm:gap-8">
                                    {/* Feature 1: High Gloss */}
                                    <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                        ...
                                        <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">Mirror Gloss</h3>
                                        <p className="text-sm text-[#493657]/70 leading-relaxed">Achieves a brilliant, glass-like reflection that enhances wood grain depth.</p>
                                    ...
                                    <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                        ...
                                        <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">Scratch Resistant</h3>
                                        <p className="text-sm text-[#493657]/70 leading-relaxed">Formulated to resist daily abrasion, keeping furniture looking new for years.</p>
                                    ...
                                    <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                        ...
                                        <p className="text-sm text-[#493657]/70 leading-relaxed">Protects against hot spills and coffee cups, preventing white rings and damage.</p>
                                    ...
                                    <div className="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-[#493657]/20 p-6 sm:p-8 hover:shadow-2xl hover:border-[#F0C85A] transition-all duration-500 hover:-translate-y-2">
                                        ...
                                        <h3 className="text-lg sm:text-xl font-bold text-[#493657] mb-2 sm:mb-3">Fast Application</h3>
                                        <p className="text-sm text-[#493657]/70 leading-relaxed">Touch dry in 30 minutes, allowing for multiple coats in a single day.</p>
                        ...
                        <div className="relative max-w-6xl mx-auto rounded-2xl sm:rounded-3xl bg-white/90 backdrop-blur-md border border-white/70 shadow-2xl px-4 py-6 sm:px-6 sm:py-10 md:px-10">
                            <div className="flex flex-col gap-4 sm:gap-6 mb-6 sm:mb-10 md:flex-row md:items-center md:justify-between">
                                <div>
                                    ...
                                    <p className="text-sm md:text-base text-[#493657]/70 mt-2">Complete your home renovation with our premium range.</p>
                                </div>
                                    ...
                                    <div className="relative h-full rounded-xl sm:rounded-2xl bg-white shadow-lg border border-[#493657]/15 overflow-hidden transition-all duration-300 group-hover:-translate-y-2 group-hover:shadow-2xl">
                                        ...
                                        <div className="flex flex-col sm:flex-row items-center p-4 sm:p-6 gap-4 sm:gap-6">
                                            <div className="flex-shrink-0 w-24 h-24 sm:w-32 sm:h-32">
                                                <img
                                                    src="/Assets/Product%20Images/Luxury%20Exterior%20Emulsion/luxury-exterior-bucket-transparent.webp"
                                ...
                                <Link to="/product/waterproofing-sealer" className="group h-full">
                                    <div className="relative h-full rounded-xl sm:rounded-2xl bg-white shadow-lg border border-[#493657]/15 overflow-hidden transition-all duration-300 group-hover:-translate-y-2 group-hover:shadow-2xl">
                                        ...
                                        <div className="flex flex-col sm:flex-row items-center p-4 sm:p-6 gap-4 sm:gap-6">
                                            <div className="flex-shrink-0 w-24 h-24 sm:w-32 sm:h-32">
                                                <img
                                                    src="/Assets/Product%20Images/Waterproof%20Sealer/waterproof-bucket-png.webp"
