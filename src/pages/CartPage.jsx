    ...
    <div className="pt-20 pb-20 px-6 md:px-12 max-w-7xl mx-auto">
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          {items.map((it, idx) => {
            const type = (it.productType || '').toLowerCase();
            const isPaint = type === 'paint' || type === 'ready-mixed' || type === 'tint-on-demand';
            return (
              <div key={idx} className="rounded-2xl bg-white border border-[#e5e0d8] p-4 flex gap-4 items-center">
                ...
                <div className="flex-1">
                  ...
                  {isPaint && it.selectedColor?.name && (
                    <div className="text-sm text-gray-600">Color: {it.selectedColor.name}</div>
                  )}
                ...
                <div className="text-right">
                  <div className="font-semibold">₹{((it.price || 0) * (it.quantity || 1)).toLocaleString('en-IN')}</div>
        ...
        <div className="rounded-2xl bg-white border border-[#e5e0d8] p-4">
          ...
          <div className="mt-4 font-bold">Total: ₹{getCartTotal().toLocaleString('en-IN')}</div>
          <button
